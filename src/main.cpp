...
uint256 hashGenesisBlock("0xdf05422cbae93dc36c58237be9282a953e702c1ebefe7cdc19cb086878f857f8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // VapeCoin: starting difficulty is 1 / 2^12
...
{
    // VapeCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // VapeCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // VapeCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // VapeCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xdf05422cbae93dc36c58237be9282a953e702c1ebefe7cdc19cb086878f857f8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1418835672;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085794911;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VapeCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VapeCoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running VapeCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VapeCoinMiner terminated\n");
        throw;
    }
}
